...
def test_txcal(txcal_data, txcal_ref_data):
    ...
    primary_header = PrimaryHeader.frombytes(phdata)
...
def test_cal_sas(txcal_data, txcal_ref_data):
    ...
    rcss = secondary_header.radar_configuration_support
    cal_sas = rcss.sas.get_sas_data()
    assert isinstance(cal_sas, SasCalData)

    ref_sh = txcal_ref_data["secondary_header"]
    ref_rcss = ref_sh.radar_configuration_support

    assert rcss.sas.get_sas_test() == ref_rcss.sas.get_sas_test()
    assert cal_sas.sas_test == ref_rcss.sas.get_sas_test()
    assert rcss.sas.get_cal_type() == ref_rcss.sas.get_cal_type()
    assert cal_sas.cal_type == ref_rcss.sas.get_cal_type()

    ref_cal_beam_address = ref_rcss.sas.get_calibration_beam_address()
    assert rcss.sas.get_calibration_beam_address() == ref_cal_beam_address
    assert cal_sas.calibration_beam_address == ref_cal_beam_address

    with pytest.raises(TypeError):
        rcss.sas.get_elevation_beam_address()
    el_beam_address = (cal_sas.sas_test << 3) | cal_sas.cal_type
    assert rcss.sas.get_elevation_beam_address(check=False) == el_beam_address

    with pytest.raises(TypeError):
        rcss.sas.get_azimuth_beam_address()
    assert (
        rcss.sas.get_azimuth_beam_address(check=False) == ref_cal_beam_address
    )
...
def test_noise_sas(noise_data, noise_ref_data):
    ...
    ref_el_beam_address = ref_rcss.sas.get_elevation_beam_address()
    assert rcss.sas.get_elevation_beam_address() == ref_el_beam_address
    assert cal_sas.elevation_beam_address == ref_el_beam_address

    ref_az_beam_address = ref_rcss.sas.get_azimuth_beam_address()
    assert rcss.sas.get_azimuth_beam_address() == ref_az_beam_address
    assert cal_sas.azimuth_beam_address == ref_az_beam_address

    with pytest.raises(TypeError):
        rcss.sas.get_sas_test()
    sas_test = (cal_sas.elevation_beam_address >> 3) & 0b00000001
    assert rcss.sas.get_sas_test(check=False) == sas_test

    with pytest.raises(TypeError):
        rcss.sas.get_cal_type()
    cal_type = cal_sas.elevation_beam_address & 0b00000111
    assert rcss.sas.get_cal_type(check=False) == cal_type

    with pytest.raises(TypeError):
        rcss.sas.get_calibration_beam_address()
    sas = rcss.sas
...
def test_echo_datation_service(echo_data, echo_ref_data):
    ...
    ref = echo_ref_data["secondary_header"].datation
...
def test_echo_radar_configuration_support_service(echo_data, echo_ref_data):
    ...
    rcss = secondary_header.radar_configuration_support
    ref = echo_ref_data["secondary_header"].radar_configuration_support
